...
-- @see opt
---
function keychains.init(globkeys,opt)
    globalkeys  = globkeys
    opt = opt or {}
...
-- 0 means no timeout.
---
function keychains.start(timeout)
...
-- @see hotkeys
---
function keychains.add(mod_hk,hk,title,icon,hotkeys,style)
    local nr = #(keychain)+1
...
---
local function get_hotkeys(which)
    local ret
...
---
function keychains.get_awful_keys(which)
    local i, hkt,ret
    local hotkeys = get_hotkeys(which)
...
---
function keychains.activite(which)
